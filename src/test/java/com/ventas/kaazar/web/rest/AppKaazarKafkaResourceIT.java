...
import com.ventas.kaazar.config.KafkaProperties;
...
class AppKaazarKafkaResourceIT {
    ...
    void setup() {
        ...
        AppKaazarKafkaResource kafkaResource = new AppKaazarKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/app-kaazar-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/app-kaazar-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
